...
function validateCredentials(login, password) {
    ...
    return login && typeof login === 'string' && login.trim() !== '' &&
           password && typeof password === 'string'; // API requires password, so check it exists
}
...
function validateSingleTrack(barcode) {
    ...
    return barcode && typeof barcode === 'string' && barcode.trim() !== '';
}

function validateBatchTrack(barcodes) {
    // Check if it's a non-empty array of non-empty strings
    return barcodes && Array.isArray(barcodes) && barcodes.length > 0 &&
           barcodes.every(bc => typeof bc === 'string' && bc.trim() !== '');
}

function validateTicket(ticket) {
    // Basic check: ensure it's a non-empty string
    return ticket && typeof ticket === 'string' && ticket.trim() !== '';
}
